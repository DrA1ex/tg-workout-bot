...
describeWithSilencedConsole('Effect Processor Tests', ['warn', 'error'], () => {
    ...
    describe('clearExistingKeyboards', () => {
        ...
        it('should handle missing ctx gracefully', async () => {
            ...
            // Should not crash
            await expect(clearExistingKeyboards(mockCtx, mockSession)).resolves.not.toThrow();
        });
    ...
    describe('processEffect', () => {
        ...
        it('should clear existing keyboards before processing new effect', async () => {
            mockSession.pending = { messageId: 123 };
            const effect = { type: 'string', text: 'test' };
            
            await processEffect(mockCtx, mockSession, 12345, effect);
            
            expect(mockCtx.telegram.editMessageReplyMarkup).toHaveBeenCalled();
        });
    });
